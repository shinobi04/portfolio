...
export default function Projects() {
  ...
  useEffect(() => {
    const fetchRepositories = async () => {
      try {
        setIsLoading(true);
        const response = await fetch('https://api.github.com/users/shinobi04/repos?sort=updated&per_page=6');
    ...
    <section id="projects" className="py-20 px-6 max-w-6xl mx-auto">
      ...
      {!isLoading && !error && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {repos.map((repo) => (
            ...
            >
              <h3 className="text-xl font-semibold mb-3 text-white">{repo.name}</h3>
              ...
              {repo.language && (
                <div className="flex items-center mb-3">
                  <span 
                    className={`h-3 w-3 rounded-full mr-2 ${languageColors[repo.language] || 'bg-gray-400'}`}
                  ></span>
      ...
      <div className="text-center mt-10">
        <a 
          ...
          target="_blank" 
          rel="noopener noreferrer"
          className="inline-flex items-center px-6 py-3 bg-gradient-to-r from-purple-500 to-pink-500 text-white font-medium rounded-full transition-all hover:shadow-[0_0_15px_rgba(168,85,247,0.5)] hover:scale-105"
        >
